...
const FeedBack = ({ onClose, setOpenFeedBack }) => {
  ...
  return (
    ...
    <div className='fixed bottom-1 left-1 right-1 z-50'>
      <div className={`bg-white shadow-xl w-full max-w-lg border border-slate-300 rounded py-12 px-3 relative md:overflow-hidden h-full max-h-[98vh] overflow-y-scroll scrollbarStyle ${openInfo ? 'hidden' : ''}`} style={{ boxShadow: "4px 4px 10px rgba(255, 255, 255, 0.3), 4px 4px 10px rgba(255, 255, 255, 0.15)" }}>
        <div className='absolute top-3 right-3'>
          <button className='text-xs cursor-pointer' onClick={onClose}>
            <GiRazor className='text-red-600 rotate-180' size={13} />
            <GiRazor className='text-red-600 -mt-2' size={13} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className='flex justify-between gap-2 mt-5 mb-5'>
            ...
            <div className='flex items-center gap-2'>
              ...
              <button type='submit' onClick={handleSubmit} disabled={selectedEmoji === null || loading} className={`${selectedEmoji === null || loading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600 cursor-pointer"} z-50 text-white py-2 px-4 text-xs rounded `}>
                {loading ? (
