...
const Calendar = () => {
  ...
  useEffect(() => {
    ...
    for (let i = firstDayOfMonth - 1; i >= 0; i--) {
      prevDays.push(new Date(year, month - 1, prevLastDate - i));
    }

    // Next month days — **only enough to complete the last week**
    const totalRendered = prevDays.length + days.length;
  ...
  return (
    <div className='fixed bottom-1 right-1 flex flex-col z-50'>
          ...
          <div className='w-full'>
            <div className='grid grid-cols-7 gap-2 mb-3 px-2'>
              ...
              {nextMonthDays.map((day, index) => (
                <div key={`next-${index}`} className='text-gray-400 text-center text-xs'>
                  {day.getDate()}
                </div>
              ))}
            </div>
